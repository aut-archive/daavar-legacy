...
function put_print_form()
{
    ...
    <table>
        <tr>
            ...
            <td><input type="file" name="code" id="code" size="40" required
                       onChange='detectLanguage(document.getElementById("code").value);'/></td>
        </tr>
        <tr>
            <td colspan="2">&nbsp;</td>
        </tr>
        <tr>
            <td><label for="langid">Language</label></td>
            <td><?php
...
function send_print($filename, $language = null, $team = null, $origname = null)
{
    ...
    $content = '<html><body><br><br><pre>' . htmlentities(file_get_contents($filename)) . '</pre></body></html>';

    $tmp = TMPDIR . '/' . time() . '_' . rand(100, 20000);
    file_put_contents($tmp . '.html', $content);

    $contest_name = $cdata['contestname'];
    $banner = "$contest_name - Team: $team  - File: $origname";

    $cmd = BINDIR . '/print ' . escapeshellarg($banner) . ' ' . escapeshellarg($tmp) . " 2>&1";
