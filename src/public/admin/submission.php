...
echo "<br/><h1 style=\"display:inline;\">Submission s" . $id .
    ($submdata['valid'] ? '' : ' (ignored)') . "</h1><br><br>";
...
?>
    ...
    <p>
        <img title="team" alt="Team:" src="../assets/images/team.png"/> <a
            href="team.php?id=<?php echo urlencode($submdata['teamid']) ?>">
            <?php echo htmlspecialchars($submdata['teamname'] . " (t" . $submdata['teamid'] . ")") ?></a>&nbsp;&nbsp;
        <img title="problem" alt="Problem:" src="../assets/images/problem.png"/> <a
            href="problem.php?id=<?php echo $submdata['probid'] ?>">
            <span class="probid"><?php echo htmlspecialchars($submdata['shortname']) ?></span>:
            <?php echo htmlspecialchars($submdata['probname']) ?></a>&nbsp;&nbsp;
        <img title="language" alt="Language:" src="../assets/images/lang.png"/> <a
            href="language.php?id=<?php echo $submdata['langid'] ?>">
            <?php echo htmlspecialchars($submdata['langname']) ?></a>&nbsp;&nbsp;
        <img title="submittime" alt="Submittime:"
             src="../assets/images/submittime.png"/> <?php echo printtime($submdata['submittime']) ?>&nbsp;&nbsp;
        <img title="allowed runtime" alt="Allowed runtime:" src="../assets/images/allowedtime.png"/>
        <?php echo htmlspecialchars($submdata['maxruntime']) ?>s&nbsp;&nbsp;
        <img title="view source code" alt="" src="../assets/images/code.png"/>
        <a href="show_source.php?id=<?= $id ?>" style="font-weight:bold;">view source code</a>

        <?php if (IS_ADMIN) {
            $val = !$submdata['valid'];
            $unornot = $val ? 'un' : '';
            echo "&nbsp;\n" . addForm('ignore.php','post','','','','style="display:inline;"') .
                addHidden('id', $id) .
                addHidden('val', $val) .
                '<input type="submit" value="' . $unornot .
                'IGNORE this submission" class="btn btn-danger btn-mini" onclick="return confirm(\'Really ' . $unornot .
...
if (isset($jid)) {
    ...
    echo "<h2 style=\"display:block;\">Judging j" . (int)$jud['judgingid'] .
        ($jud['valid'] == 1 ? '' : ' (INVALID)') .
        "</h2>\n\n&nbsp;";
    if (!$jud['verified']) {
        ...
        if ($jury_member == @$jud['jury_member']) {
            echo addSubmit('unclaim', 'unclaim',null,true,'class="btn"');
        } else {
            echo addSubmit('claim', 'claim',null,true,'class="btn"');
        }
        echo addEndForm();
    }
    ...
    echo($lastjud === NULL ? '' :
        "<br><br><span class=\"testcases_prev\">" .
    ...
    if ($judging_ended) {
        ...
        if ($jud['valid']) {
            ...
            if (!($verification_required && $jud['verified'])) {
                echo '<br><br>' . addSubmit(($val ? '' : 'un') . 'mark verified', 'verify',null,true,'class="btn"');
    ...
    foreach ($runinfo as $run) {
        ...
        if (strlen(@$run['output_run']) > 0) {
            echo "<textarea readonly='readonly' rows='10' class=\"output_text\">" .
                htmlspecialchars($run['output_run']) . "</textarea>\n\n";
        } else {
