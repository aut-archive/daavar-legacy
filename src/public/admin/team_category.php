...
if (!empty($_GET['cmd'])):
    ...
    <tr>
        <td><label for="data_0__name_">Description:</label></td>
        <td><?php echo addInput('data[0][name]', @$row['name'], 15, 255, 'required') ?></td>
    </tr>

    <tr>
        <td><label for="data_0__sortorder_">Sort order:</label></td>
        <td><?php echo addInputField('number', 'data[0][sortorder]', (empty($row['sortorder']) ? 0 : $row['sortorder']), ' size="3" maxlength="2"') ?></td>
    </tr>

    <tr>
        <td><label for="data_0__color_">Colour:</label></td>
        <td><?php echo addInputField('color', 'data[0][color]', @$row['color'],
            ...
            <a target="_blank"
               href="http://www.w3schools.com/cssref/css_colornames.asp"><img
                    src="../assets/images/b_help.png" class="smallpicto" alt="?"/></a></td>
    </tr>

    <tr>
        <td>Visible:</td>
        <td><?php echo addRadioButton('data[0][visible]', (!isset($row['visible']) || $row['visible']), 1) ?> <label
                for="data_0__visible_1">yes</label>
            <?php echo addRadioButton('data[0][visible]', (isset($row['visible']) && !$row['visible']), 0) ?> <label
                for="data_0__visible_0">no</label></td>
    </tr>
