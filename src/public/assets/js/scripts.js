            ...
            for (i = 0; i < nclars; i++) {
                sendNotification('New clarification.',
                    {'tag': 'clar_' + resp.clarifications[i].clarid,
                        'link': 'clarification?id=' + resp.clarifications[i].clarid,
...
//	if ( typeof options.tag === 'undefined' ) options.tag = null;
    ...
    not.onshow = function () {
        setTimeout(not.close, timeout * 1000);
    }
...
function detectProblemLanguage(filename) {
    ...
    if (elt.value != '' && elt.touched) return;

    for (i = 0; i < elt.length; i++)
        if (parts[1] == elt.options[i].text.toLowerCase().split(' : ')[0])
            elt.selectedIndex = i;
...
//function boostrapConfirm(question) {
//
//    $('#confirm-body').text(question);
//
//    $('#confirm').modal('show');
//
//}
