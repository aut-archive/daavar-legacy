                               ...
                               $limitteams = null, $displayrank = TRUE, $center = FALSE, $showlegends = TRUE)
{
    ...
    if ($showlegends) {
        ...
        foreach ($cellcolors as $color => $desc) {
            ...
            echo "<span class='badge score_$color'>$desc</span>  ";
        }
    }
    echo '<br><br>';
    ...
    echo '<table class="scoreboard table table-bordered' . (IS_JURY ? ' scoreboard_jury' : '') . ($center ? ' center' : '') . "\">\n";
    ...
    echo '<tr class="scoreheader">' .
        ...
        ($SHOW_AFFILIATIONS ? '<th>aff</th>':'').'<th title="team name" scope="col"' .
    ...
    foreach ($probs as $pr) {
        ...
        if (IS_JURY || $pr['hastext'] > 0) {
            echo '<a href="problem?id=' . urlencode($pr['probid']) .
                '">' . $str . '</a></th>';
        } else {
            echo '<a>' . $str . '</a></th>';
        }
    ...
    foreach ($scores as $team => $totals) {
        ...
        echo
            ...
            ($SHOW_AFFILIATIONS ? '<span class="univ">' . $affilname .
                '</span>' : '') .
            ($static ? '' : '</a>') .
            '</td>';
        echo
    ...
    if (empty($limitteams)) {
        ...
        foreach (array_keys($probs) as $prob) {
            ...
            echo '<td>' .
                jurylink('problem?id=' . urlencode($prob), $str) .
                '</td>';
        }
        echo "</tr>\n</tbody>\n";
    }
...
function putScoreBoard($cdata, $myteamid = NULL, $static = FALSE, $filter = FALSE)
{
    ...
    if ($fdata['showfinal']) {
        echo "<h3 class='nodata'>Final standings</h3>\n\n";
    } elseif (!$fdata['cstarted']) {
        echo "<h3>" . printContestStart($cdata) . "</h3>\n\n";
        ...
        echo "<h4 class='nodata'>contest starts at " . printtime($cdata['starttime']) .
            " and ends at " . printtime($cdata['endtime']).' ...';
...
//				echo '<a href="../public/">the public scoreboard</a> is ';
    ...
    if ($filter !== FALSE && $static !== TRUE) {
        ...
        <table class="scorefilter">
            <tr>
                <td><a href="javascript:collapse('filter')"><img src="../assets/images/filter.png"
                                                                 alt="filter&hellip;" title="filter&hellip;"
                                                                 class="picto"/></a></td>
                <td>
                    <div id="detailfilter">
                        <?php
...
function putTeamRow($cdata, $teamids)
{
    ...
    if (count($teamids) == 1) {
        ...
        $MATRIX = array();
        $scoredata = $DB->q('SELECT * FROM scorecache_jury WHERE cid = %i AND teamid IN (%A' . 's)', $cid, $teamids);
